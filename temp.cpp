...
namespace cost{
	double alpha = 0; 	//obstacle
	double beta = 0.1;	//path
	double gamma = 0;	//goal

//	double alpha = 10; 	//obstacle
//	double beta = 25;	//path
//	double gamma = 5;	//goal
//	
...
twist get_best_action(
	...
	// Calculate the maximum distance the robot can move in dt seconds
	double max_distance = max_v * dt;
...
int main(){
	...
	graph space(finer_grid);
	space.resolution = resolution;

	double dt = 1;
	vector<point> path = space.a_star(0, w*h/resolution/resolution -1);

	while (cost::Goal(bot, goal) > 1){
		twist action = get_best_action(bot, occupancy_grid, path, goal, dt, resolution);
